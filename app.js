...
async function addHtmlPageToPdf(pdfDoc, htmlEl) {
  ...
  // Questo evita problemi con transform:scale, container con overflow, viewport mobile.

  const A4_WIDTH_PX = 794;   // 210mm a 96dpi
  const A4_HEIGHT_PX = 1123; // 297mm a 96dpi

  // Container temporaneo posizionato fuori dalla viewport visibile
  ...
  await Promise.all(
    cloneImages.map(async (img) => {
      if (img.src && !img.src.startsWith('data:')) {
        ...
        img.src = dataUrl;
        // Forzo crossOrigin per sicurezza
  ...
  try {
    ...
    // Piccola pausa per dare al browser il tempo di calcolare il layout
    ...
    // pdf-lib può detachare il buffer durante embedJpg, quindi non condividiamo
    // mai lo stesso ArrayBuffer (che invaliderebbe operazioni successive).
