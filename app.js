...
async function generatePDF() {
  ...
  try {
    // Forzo il render delle pagine custom
    renderPreview();
    ...
    toast('Errore generazione PDF: ' + detail, 'error', 6000);
  }
}
...
async function addHtmlPageToPdf(pdfDoc, htmlEl) {
  ...
  try {
    ...
    const imgBytes = new Uint8Array(arrayBuffer.byteLength);
    imgBytes.set(new Uint8Array(arrayBuffer));
...
async function addPdfsFromSection(finalPdf, sectionKey) {
  ...
  for (const file of files) {
    try {
      // Preferisco i bytes "vivi" se presenti, altrimenti decodifico dal base64 salvato
      let bytes;
      if (file.bytes instanceof Uint8Array) {
        // Clono i bytes: pdf-lib può consumarli
