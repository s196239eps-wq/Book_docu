...
function syncStateToForm() {
  ...
  renderMaterialiRows();
  renderAllFileLists();
  if (typeof updateTimbroPreview === 'function') updateTimbroPreview();
}
...
async function handleUpload(event, section) {
  ...
  for (const file of files) {
    try {
      ...
      // Memorizzo direttamente come Uint8Array (no conversione base64 in memoria)
      // La conversione base64 avviene solo al salvataggio in localStorage
      const bytes = new Uint8Array(arrayBuffer);

      // Validazione veloce: i PDF iniziano con "%PDF-"
      const header = String.fromCharCode(...bytes.slice(0, 5));
      if (header !== '%PDF-') {
        toast(`${file.name}: non è un PDF valido`, 'error');
        errCount++;
        continue;
      }
      ...
      currentBook.pdfs[section].push({
        name: file.name,
        size: file.size,
        bytes: bytes,        // dati binari "vivi" in memoria per questa sessione
        data: null           // base64 generato on-demand per il salvataggio
      });
      okCount++;
    } catch (err) {
  ...
  event.target.value = '';
  if (okCount > 0) toast(`${okCount} PDF caricato${okCount > 1 ? 'i' : ''}`, 'success');
}
...
// ==========================================================================
// TIMBRO CUSTOM (sezione APPALTATORE della tabella materiali)
// ==========================================================================

async function handleTimbroUpload(event) {
  ...
  // Limito a 2 MB per non saturare il localStorage
  if (file.size > 2 * 1024 * 1024) {
    toast('Immagine troppo grande (max 2 MB)', 'error');
    return;
  }
  ...
  try {
    ...
    currentBook.customTimbro = dataUrl;
    updateTimbroPreview();
    toast('Timbro caricato', 'success');
  } catch (err) {
    toast('Errore caricamento timbro: ' + err.message, 'error');
  }

  event.target.value = '';
}

function resetTimbro() {
  if (!confirm('Ripristinare il timbro VED predefinito?')) return;
  currentBook.customTimbro = null;
  updateTimbroPreview();
  toast('Timbro ripristinato', 'info');
}

function updateTimbroPreview() {
  const preview = document.getElementById('timbroPreview');
  const label = document.getElementById('timbroPreviewLabel');
  const resetBtn = document.getElementById('resetTimbroBtn');
  if (!preview) return;

  if (currentBook.customTimbro) {
    preview.src = currentBook.customTimbro;
    if (label) label.textContent = 'Timbro personalizzato in uso:';
    if (resetBtn) resetBtn.style.display = 'inline-flex';
  } else {
    preview.src = 'assets/timbro_ved.png';
    if (label) label.textContent = 'Timbro attualmente in uso (predefinito):';
...
function buildCoverPage() {
  ...
  page.innerHTML = `
    <div class="cover-header">
      <div class="cover-header-redbar">
        <span class="cover-header-tms-text">Technical Maintenance Services</span>
      </div>
      <img src="assets/logo_tms.png" class="cover-header-tms-logo" alt="TMS">
      <img src="assets/logo_ved.png" class="cover-header-ved-logo" alt="VED">
    </div>
...
function buildMaterialiPage() {
  ...
  page.innerHTML = `
    ...
    <div class="materiali-signatures">
      <div>
        APPALTATORE
        <img src="${currentBook.customTimbro || 'assets/timbro_ved.png'}" class="timbro-ved" alt="Timbro Appaltatore">
      </div>
      <div>CLIENTE</div>
...
async function generatePDF() {
  // Lock per evitare doppio click che causa race condition sui buffer
  if (_isGeneratingPdf) {
    toast('Generazione già in corso, attendi...', 'info');
    return;
  }

  // Pre-flight: verifico che le librerie esterne siano caricate
  if (typeof PDFLib === 'undefined' || !PDFLib.PDFDocument) {
    toast('Libreria PDF non ancora caricata. Riprova fra qualche secondo.', 'error', 5000);
    return;
  }
  if (typeof html2canvas === 'undefined') {
    toast('Libreria html2canvas non ancora caricata. Riprova fra qualche secondo.', 'error', 5000);
    return;
  }
  ...
  try {
    // Forzo il render delle pagine custom (fresco ad ogni chiamata)
    renderPreview();
    // Attendo che il browser completi layout e caricamento immagini
    ...
    console.error('Errore PDF dettagliato:', err);
    setLoading(false);
    // Mostro più dettagli dell'errore per facilitare il debug
    const detail = err && err.message ? err.message : String(err);
    let userMsg = 'Errore generazione PDF: ' + detail;
    // Se è il classico errore "DataView" suggerisco la soluzione
    if (detail.includes('DataView') || detail.includes('detached')) {
      userMsg = 'Errore PDF (buffer corrotto). Ricarica la pagina (F5) e riprova.';
    }
    toast(userMsg, 'error', 7000);
  } finally {
    _isGeneratingPdf = false;
  }
}

// Cache delle immagini convertite in dataURL (per evitare di ri-fetchare ogni volta)
const _imgDataUrlCache = new Map();

async function imgSrcToDataUrl(src) {
  // Se è già un dataURL, ritornalo così
  ...
  try {
    ...
    if (!response.ok) throw new Error('Fetch immagine fallito: ' + response.status);
    ...
    _imgDataUrlCache.set(src, dataUrl);
    return dataUrl;
  } catch (err) {
    console.warn('Impossibile convertire immagine in dataURL:', src, err);
    return src;  // fallback all'src originale
  }
}

async function addHtmlPageToPdf(pdfDoc, htmlEl) {
  // STRATEGIA: clono la pagina in un container fuori schermo a dimensione A4 piena.
  // IMPORTANTE: NON usiamo visibility:hidden o display:none perché html2canvas
  // non riesce a renderizzare elementi nascosti correttamente.
  // Posizioniamo il container LONTANO dalla viewport con left negativo.

  const A4_WIDTH_PX = 794;   // 210mm a 96dpi
  const A4_HEIGHT_PX = 1123; // 297mm a 96dpi

  // Container temporaneo fuori dallo schermo (ma renderizzato)
  ...
  // Clono profondamente la pagina
  const clone = htmlEl.cloneNode(true);
  // Reset di tutte le trasformazioni e dimensioni vincolanti
  ...
  // CONVERTO TUTTE LE IMMAGINI INTERNE IN DATAURL (bypassa CORS e Service Worker)
  ...
  try {
    // Aspetto che le immagini del clone siano ufficialmente caricate
    await Promise.all(
      cloneImages.map((img) => {
    ...
    // Pausa più lunga per dare al browser il tempo di calcolare layout, font, dimensioni
    await new Promise((r) => setTimeout(r, 400));

    // Verifica dimensioni reali del clone prima di passarlo a html2canvas
    const realWidth = clone.offsetWidth || A4_WIDTH_PX;
    const realHeight = Math.max(clone.offsetHeight, A4_HEIGHT_PX);

    if (realWidth === 0 || realHeight === 0) {
      throw new Error(`Pagina ha dimensioni zero (${realWidth}x${realHeight})`);
    }
    ...
    if (!canvas || canvas.width === 0 || canvas.height === 0) {
      throw new Error(`Canvas generato vuoto (${canvas?.width || 0}x${canvas?.height || 0})`);
    }

    // Converto il canvas in Uint8Array tramite blob
    const blob = await new Promise((resolve, reject) => {
      canvas.toBlob(
        (b) => {
          if (b && b.size > 0) resolve(b);
          else reject(new Error('toBlob ha prodotto un blob vuoto - probabile problema CORS'));
        },
        'image/jpeg',
        0.92
      );
    ...
    // CRITICO: copia REALE con nuovo ArrayBuffer indipendente.
    const imgBytes = new Uint8Array(arrayBuffer.slice(0));
...
async function addPdfsFromSection(finalPdf, sectionKey) {
  ...
  for (const file of files) {
    try {
      // CRITICO: ottengo SEMPRE un nuovo Uint8Array con ArrayBuffer indipendente.
      // pdf-lib può detachare il buffer durante load(), quindi non possiamo
      // condividere lo stesso ArrayBuffer con file.bytes (lo distruggerebbe).
      let bytes;
      if (file.bytes instanceof Uint8Array && file.bytes.byteLength > 0) {
        // .slice() crea una vera copia con un NUOVO ArrayBuffer
        bytes = file.bytes.slice();
      } else if (file.data) {
        bytes = base64ToUint8Array(file.data);
      } else {
        throw new Error('Dati PDF mancanti');
      }

      // Validazione header PDF
      const header = String.fromCharCode(...bytes.slice(0, 5));
      if (header !== '%PDF-') {
        throw new Error('Non è un PDF valido');
      }
      ...
      toast(`Saltato ${file.name}: ${err.message}`, 'error', 4000);
      // Continua con gli altri file invece di fermare tutta la generazione
    }
  }
}

// Conversione base64 → Uint8Array sicura (evita problemi con caratteri binari)
...
function saveBook() {
  ...
  // Preparo una copia serializzabile: converto i Uint8Array in base64 stringhe
  // (JSON.stringify non gestisce i Uint8Array)
  const serializable = {
    ...currentBook,
    pdfs: {}
  };
  for (const [section, files] of Object.entries(currentBook.pdfs)) {
    serializable.pdfs[section] = files.map((f) => ({
      name: f.name,
      size: f.size,
      data: f.data || (f.bytes ? uint8ArrayToBase64(f.bytes) : null)
    }));
  }
  // Aggiungo anche il timbro custom (se caricato)
  if (currentBook.customTimbro) {
    serializable.customTimbro = currentBook.customTimbro;
  }

  const idx = books.findIndex((b) => b.id === currentBook.id);
  if (idx >= 0) {
    books[idx] = serializable;
  } else {
    books.push(serializable);
  }
...
// Conversione Uint8Array → base64 sicura (chunked per evitare stack overflow)
...
function loadBook(id) {
  ...
  // Ricostruisco i bytes vivi dai base64 salvati (lazy: solo se servono per il PDF)
  // I .data restano disponibili come fallback
  for (const section of Object.keys(currentBook.pdfs)) {
    currentBook.pdfs[section] = (currentBook.pdfs[section] || []).map((f) => ({
      name: f.name,
      size: f.size,
      data: f.data || null,
      bytes: null  // non li deserializzo subito; vengono ricostruiti quando servono
    }));
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.handleTimbroUpload = handleTimbroUpload;
  window.resetTimbro = resetTimbro;
});
