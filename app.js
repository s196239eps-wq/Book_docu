...
async function addHtmlPageToPdf(pdfDoc, htmlEl) {
  ...
  try {
    // Aspetto che TUTTE le immagini interne siano completamente caricate
    // (importante al secondo run, quando potrebbero esserci immagini in fase di re-fetch)
    ...
    await Promise.all(
      Array.from(images).map((img) => {
        ...
        return new Promise((resolve) => {
          ...
          // Timeout di sicurezza: non aspettare più di 3 secondi per immagine
    ...
    // Converto il canvas in Uint8Array passando per blob (più affidabile di dataURL+fetch
    // ed evita problemi di buffer detached con pdf-lib)
    ...
    // Creo un Uint8Array nuovo (copia), così pdf-lib non riceve un buffer "condiviso"
